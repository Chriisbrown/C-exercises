...
{
    double timing_error = 0.25; //Error in the drift times
    ...
    double final_fit_tolerance = 0.00001; // Final tolerance, the velocity must change by less than this value in fit iterations in order to be a good fit
...
//Simple squaring function to reduce code complexity, used for either double or int so is left as a template 
    ...
    if (is)
    {
        ...
        for (int j=0;j<buffer_size;++j)
        {
            ...
            for (int i=0;i<8;++i)
            {
                ...
                hit h1(start*buffer_size*8+j*8+i,x_pos,Y_adjusted,(bitextractor(val,10,6)/2)+0.25);
                // create a hit, event id given by where in the total file this hit has been read from
                // the final 10 bits are converted to the time and placed in the middle of the 0.5ns bins
...
void weighted_least_squares(event& e, double (*f)(double), bool hit,fitting_parameters& fp)
{
    ...
    */
    ...
    //error calcualtion from velocity error propagation

    double standard_Error = std::sqrt(1/X_var);

    e.update_fit(intercept,gradient,standard_Error);
    //Write new values of the fit to event
    for (int i=0; i<n; ++i)
    {
...
void drift_time_calculation(event& e, fitting_parameters& fp)
{
    ...
    */
    ...
    double v_error = (fp.timing_error/mean_t)*(speed/mean_t);
    e.update_velocity(speed/mean_t,v_error);
}
...
event track_finder(event& E, fitting_parameters& fp)
{
    ...
    */
    ...
    while (!E.check_fit_status()){
        ...
        hit_finder(E);
        weighted_least_squares(E,no_weight,true,fp);
        drift_time_calculation(E,fp);
